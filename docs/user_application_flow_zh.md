...
每个采样值都会完成滤波和物理量换算。固件只抽取外发遥测：根据真实轮询吞吐和活动 ADC 数量计算抽取倍数，将遥测限制在约 `3000 frames/s`。
...
所有业务帧均使用标准 11-bit ID、CAN FD+BRS、`500K / 2M` 和 XOR 校验。

| 方向 | ID | 类型 | 长度 | 用途 |
|---|---:|---:|---:|---|
| PC -> MCU | `0x100` | `0xA0` | 8 | 命令 |
| MCU -> PC | `0x101` | `0x51` | 8 | 单通道遥测 |
...
byte 0: 类型 0x51
byte 1: 逻辑通道 0..7
byte 2-3: 电压 int16 BE，单位 0.01 mV
byte 4-5: 应变 int16 BE，单位 microstrain
byte 6: 应力预览 int8，单位 0.1 MPa，溢出时饱和
byte 7: bytes 0..6 的 XOR CRC
```
